...
export default class TeamManagerDashboardPage {
  ...
  constructor(page: Page) {
    ...
    this.advanced_filter_label = this.page.getByRole('button', {
      name: this.teamManagerDashboardPageTestData.Team_Manager_Dashboard_Page.advanced_filter_label,
    });
    ...
    this.day_from_text = this.date_from_filter_input.getByLabel(
      this.teamManagerDashboardPageTestData.Filter_Labels.day_from_label
    );
    this.month_from_dropdown = this.date_from_filter_input.getByLabel(
      this.teamManagerDashboardPageTestData.Filter_Labels.month_from_label
    );
    this.year_from_text = this.date_from_filter_input.getByLabel(
      this.teamManagerDashboardPageTestData.Filter_Labels.year_from_label
    );
    this.date_to_filter_input = this.page.getByTestId('Search_ToDate_date');
    this.day_to_text = this.date_to_filter_input.getByLabel(
      this.teamManagerDashboardPageTestData.Filter_Labels.day_to_label
    );
    this.month_to_dropdown = this.date_to_filter_input.getByLabel(
      this.teamManagerDashboardPageTestData.Filter_Labels.month_to_label
    );
    ...
    this.iras_id_search_text = this.page.getByLabel(
      this.teamManagerDashboardPageTestData.Team_Manager_Dashboard_Page.iras_id_search_box_label,
      { exact: true }
    );
  }
...
`);
  ...
  async sqlGetModificationByLeadNationAndStatusCount(lead_nation: string, status: string, countValue: string) {
    const option = 'project_record_answer_option_lead_nation_' + lead_nation.toLowerCase();
    const leadNationOption = teamManagerDashboardPageTestData.Team_Manager_Dashboard_Page[option];
    const sqlConnection = await connect(dbConfigData.Application_Service);
    const queryResult = await sqlConnection.query(`
...
`);
  ...
  async sqlGetIrasIdByLeadNationAndStatusCountSWR(
    ...
    NationQuery.CreatedDate,
    NationQuery.[Status],
    NationQuery.ReviewerName
FROM (
    SELECT
        ProjectModifications.ModificationIdentifier,
        ProjectModifications.CreatedDate,
        ProjectModifications.[Status],
         ProjectModifications.ReviewerName,
        ProjectRecords.Id,
        ProjectRecords.IrasId,
        ProjectRecordAnswers.ProjectRecordId,
        ProjectRecordAnswers.QuestionId,
    ...
    WHERE
        ...
        ProjectRecordAnswers.SelectedOptions ='${leadNationOption}' AND
        ProjectRecords.[Status] = 'Active' AND
        ProjectModifications.[Status] = '${status}' AND ProjectModifications.ReviewerName = '${swrName}'
) AS NationQuery
INNER JOIN ProjectRecordAnswers
    ON ProjectRecordAnswers.ProjectRecordId = NationQuery.Id
WHERE
...
`);
  ...
  async saveModificationIdTM(modificationId: string, countval: string) {
  ...
  async updateIrasIdTestDataJson(filePath: string, updateVal: string, countval: string) {
    (async () => {
      try {
        const data = await returnDataFromJSON(filePath);
        if (countval === 'Single') {
          data.Search_Queries.IRAS_ID_Study_Wide_Reviewer_Date_Range_Single.search_input_text = updateVal;
        } else if (countval === 'Partial') {
