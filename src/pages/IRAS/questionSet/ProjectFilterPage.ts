...
import * as projectFilterPageTestData from '../../../resources/test_data/iras/question_set/pages/project_filter_page_data.json';
...
export default class ProjectFilterPage {
  ...
  constructor(page: Page) {
    ...
    this.bioresource_established_nhs_hsc_radio = this.project_scope_section.getByTestId(/^IQA0015_OPT/);
    this.is_ctimp_radio = this.project_scope_section.getByTestId(/^IQA0017_OPT/);
    this.options_applied_for_project_radio = this.project_scope_section.getByTestId(/^IQA0018_OPT/);
    ...
    this.exposing_ionising_radiation_radio = this.project_scope_section.getByTestId(/^IQA0021_OPT/);
    this.using_human_biological_samples_radio = this.project_scope_section.getByTestId(/^IQA0022_OPT/);
    this.patient_data_accessed_outside_radio = this.project_scope_section.getByTestId(/^IQA0024_OPT/);
    this.research_with_adults_lacking_capacity_to_consent_radio =
      this.project_scope_section.getByTestId(/^IQA0025_OPT/);
    this.include_children_radio = this.project_scope_section.getByTestId(/^IQA0026_OPT/);
    ...
    this.where_human_sample_obtained_checkbox = this.research_location_section.getByTestId(/^IQA0034_OPT/);
  ...
  async getQSetSectionLocator(sectionName: string): Promise<Locator> {
    ...
    if (sectionName === 'Project_Details') {
      sectionLocator = this.project_details_section;
    } else if (sectionName === 'Project_Scope') {
      sectionLocator = this.project_scope_section;
    } else if (sectionName === 'Research_Location') {
      sectionLocator = this.research_location_section;
    } else {
      throw new Error(
        `${sectionName} is not defined in the project filter page. Create a locator for the section - ${sectionName}`
      );
    }
