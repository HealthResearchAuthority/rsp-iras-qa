...
import * as studentPageTestData from '../../../resources/test_data/iras/question_set/pages/student_page_data.json';
...
export default class StudentPage {
  ...
  async getQSetSectionLocator(sectionName: string): Promise<Locator> {
    ...
    if (sectionName === 'Educational_Project') {
      sectionLocator = this.educational_project_section;
    } else {
      throw new Error(
        `${sectionName} is not defined in the student page. Create a locator for the section - ${sectionName}`
      );
    }
