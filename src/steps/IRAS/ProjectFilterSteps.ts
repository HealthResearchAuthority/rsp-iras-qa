...
Then(
  ...
  async ({ projectFilterPage, commonItemsPage }, datasetName: string) => {
    const dataset = projectFilterPage.projectFilterPageTestData[datasetName];
    for (const key in dataset) {
      if (Object.prototype.hasOwnProperty.call(dataset, key)) {
        if (dataset[key].type === 'text') {
          const projDetailsTextboxLocator = projectFilterPage.projectDetailsTextInputField.and(
            projectFilterPage.page.getByTestId(dataset[key].id)
          );
          await projDetailsTextboxLocator.fill(dataset[key].value);
        } else {
          await commonItemsPage.selectRadio(
            projectFilterPage.projectDetailsFormGroupLabel,
            dataset[key].label,
            dataset[key].id
          );
        }
